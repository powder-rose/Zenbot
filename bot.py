...
from cloudpayments_service import CloudPaymentsWebhookServer
...
tenant_dzen_manager: TenantDzenManager | None = None
...
def _responder_progress_text(runtime: dict, *, week: bool) -> str:
    stage = str(runtime.get("stage") or "")
    done = int(runtime.get("done") or 0)
    total = int(runtime.get("total") or 0)
    if stage == "scan":
        return "🗓 Сканирую комментарии за последние 7 дней…" if week else "🔎 Сканирую новые комментарии Дзена…"
    if stage == "generating":
        suffix = f" {done}/{total}" if total else ""
        return f"🧠 Генерирую ответы через YandexGPT…{suffix}"
    if stage == "posting":
        suffix = f" {done}/{total}" if total else ""
        return f"💬 Публикую ответы в Дзене…{suffix}"
    return "⏳ Responder выполняет обработку…"


async def _await_responder_task(msg: Message, task: asyncio.Task, *, week: bool) -> dict:
    last_text = ""
    while not task.done():
        if comment_responder_worker is not None:
            runtime = comment_responder_worker.runtime_status()
            text = _responder_progress_text(runtime, week=week)
            if text != last_text:
                try:
...
async def _show_responder_result(msg: Message, result: dict, *, week: bool = False) -> None:
    ...
    if state == "busy":
        ...
        await msg.edit_text(
            "⏳ Responder уже выполняет другую проверку. Повторный запуск не создан.\n\n"
            + _responder_progress_text(runtime, week=str(runtime.get("scope") or "") == "week")
        )
        return
...
async def cb_dzen_responder_run_now(call: CallbackQuery):
    ...
    try:
        task = asyncio.create_task(comment_responder_worker.run_once(force=True))
        result = await _await_responder_task(msg, task, week=False)
    except Exception as exc:
...
async def cb_dzen_responder_week_confirm(call: CallbackQuery):
    ...
    try:
        task = asyncio.create_task(comment_responder_worker.run_week(force=True))
        result = await _await_responder_task(msg, task, week=True)
    except Exception as exc:
...
async def main():
    ...
    cloudpayments_webhook = CloudPaymentsWebhookServer(
        bot=bot,
    )
    await cloudpayments_webhook.start()
    ...
    try:
        ...
        await cloudpayments_webhook.stop()
